...
#define API_VERSION "1.0.0.0"                        // Version
...
TPCANStatus CAN_GetStatus(TPCANHandle Channel) {
	...
	try {
		...
		case CAN_ERR_QOVERRUN:
			Result = PCAN_ERROR_QOVERRUN;
			break;
		case CAN_ERR_QXMTFULL:
			Result = PCAN_ERROR_QXMTFULL;
			break;
...
TPCANStatus CAN_Read(TPCANHandle Channel, TPCANMsg* MessageBuffer,
	...
	try {
		...
		case MSGTYPE_EXTENDED:
			...
			break;
		case MSGTYPE_RTR:
			MessageBuffer->MSGTYPE = PCAN_MESSAGE_RTR;
			break;
		case MSGTYPE_STATUS:
			MessageBuffer->MSGTYPE = PCAN_MESSAGE_STATUS;
			break;
		default:
			MessageBuffer->MSGTYPE = PCAN_MESSAGE_STANDARD;
		}
...
TPCANStatus CAN_Write(TPCANHandle Channel, TPCANMsg* MessageBuffer) {
	...
	try {
		...
		wrm.MSGTYPE = MessageBuffer->MSGTYPE;
		switch (MessageBuffer->MSGTYPE) {
		case PCAN_MESSAGE_STANDARD:
			wrm.MSGTYPE = MSGTYPE_STANDARD;
			break;
		case PCAN_MESSAGE_EXTENDED:
			wrm.MSGTYPE = MSGTYPE_EXTENDED;
			break;
...
TPCANStatus CAN_GetValue(TPCANHandle Channel, TPCANParameter Parameter,
	...
	try {
		...
		case PCAN_DEVICE_NUMBER:
			...
			*((int*) Buffer) = Params.func.dwSerialNumber;
			break;
