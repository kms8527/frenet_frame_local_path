...
try:
    cdll = CDLL("/home/a/frenet_path_ws/src/build/libFrenetOptimalTrajectory.so")
except:
...
def run_fot(initial_conditions, hyperparameters):
    ...
    Args:
        initial_conditions (dict): dict containing the following items
            ...
            wp (np.ndarray([float, floaoat, float])): list of global waypoints
            obs (np.ndarray([flt, float, float])): list of obstacles
